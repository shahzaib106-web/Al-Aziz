...
function CartInner() {
  ...
  return (
    <div dir="ltr" className="mx-auto max-w-md min-h-screen bg-cream pb-24 shadow-xl flex flex-col">
      <PageHeader
        title="Your Cart"
        right={
          items.length > 0 ? (
            <button onClick={clear} className="text-[11px] font-semibold tracking-wide hover:underline px-1">
              Clear
            </button>
          ) : null
        }
          ...
          <div className="p-4 space-y-3 flex-1">
            {items.map((i) => (
              <div key={i.key} className="card p-3 flex items-center gap-3">
                <img src={i.image} alt={i.nameEn} className="w-16 h-16 rounded-lg object-cover border border-[#E8DCC3]" />
