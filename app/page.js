      ...
      <div dir={isUr ? 'rtl' : 'ltr'} className="mx-auto max-w-md md:max-w-none min-h-screen bg-cream pb-[calc(90px+env(safe-area-inset-bottom))] md:pb-10 shadow-xl md:shadow-none relative flex flex-col">
        ...
        <div className="px-4 mt-3.5 md:mt-8 md:max-w-3xl md:mx-auto md:w-full flex items-center gap-2.5">
          ...
          >
            ...
            {activeFilters > 0 && (
              <span key={activeFilters} className="pop absolute -top-1 -right-1 w-5 h-5 rounded-full bg-gold text-[#3E2C05] text-[10px] font-extrabold flex items-center justify-center ring-2 ring-cream">
                {activeFilters}
              </span>
            )}
          </button>
              ...
              <div className="relative overflow-hidden rounded-2xl shadow-card">
                <div key={slide} className="relative h-[196px] md:h-64 hero-in">
                  ...
                  <div className="relative h-full flex flex-col justify-center p-4 sm:p-5 pe-3 sm:pe-[32%] md:pe-[45%] max-w-xl">
                    <span suppressHydrationWarning className="text-gold text-[10px] font-extrabold uppercase tracking-[0.14em] drop-shadow">{s.accent}</span>
                    <h2 suppressHydrationWarning className={`${isUr ? 'urdu text-lg leading-loose' : 'text-[20px] sm:text-[22px] font-extrabold leading-[1.15]'} mt-1 text-white drop-shadow-md`}>{s.title}</h2>
                    <p suppressHydrationWarning className={`text-[11px] text-white/90 mt-1 drop-shadow ${isUr ? 'urdu leading-relaxed' : 'leading-snug'} line-clamp-2`}>{s.sub}</p>
                    <Link suppressHydrationWarning href={s.href} className={`btn btn-sm !h-8.5 sm:!h-9 w-max mt-2.5 sm:mt-3 bg-gold text-[#3E2C05] hover:brightness-110 rounded-full px-4 sm:px-5 shadow-lg ${isUr ? 'urdu' : '!tracking-wide'}`}>
          ...
          <div className="gold-banner rounded-2xl p-3.5 flex items-center gap-3 shadow-card border border-[#C9971C]/40">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-baseline gap-2 mt-0.5">
                <span className={`${isUr ? 'urdu' : ''} text-base font-extrabold text-[#4A3608]`}>{settings?.discountPercent ?? 10}% {t('رعایت', 'OFF')}</span>
              ...
              <Link href="/menu" className={`btn btn-sm !h-8 w-max mt-2.5 bg-maroon text-white rounded-full px-4 text-[11px] shadow-md ${isUr ? 'urdu' : '!tracking-wide'}`}>
                {t('ابھی آرڈر کریں', 'ORDER NOW')} <Icon name="chevR" className="w-3 h-3" strokeWidth={2.6} />
              </Link>
            </div>
            <img src="/img/tandoor.jpg" alt="" className="w-[72px] h-[72px] rounded-xl object-cover border-2 border-white/80 shadow-md shrink-0" />
          </div>
        </div>
