...
export function BottomNav({ active }) {
  ...
  const tabs = [
    ...
    { id: 'cart', en: 'Cart', ur: 'کارٹ', icon: 'cart', href: '/cart' },
    ...
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-40 bg-white border-t border-[#E8DCC3] md:hidden pb-safe">
      <div className="grid grid-cols-5">
        {tabs.map((tab) => {
          ...
          return (
            <Link key={tab.id} href={tab.href} className={`relative flex flex-col items-center gap-0.5 py-2 text-[10px] font-semibold transition ${on ? 'text-maroon' : 'text-muted hover:text-ink'}`}>
